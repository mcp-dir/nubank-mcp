...
### Nubank in Claude, ChatGPT and AI agents via Open Finance

Connect your **Nubank** account to Claude, ChatGPT and AI agents via Brazil's Open Finance. Ask about Nubank balances, statements, credit-card bills and investments in natural language. Regulated by the Central Bank, read-only.

- 🏦 **Nubank via Brazil's Open Finance**, regulated by the Central Bank
...
[➕ Open in Claude and connect](https://claude.ai/customize/connectors?modal=add-custom-connector&mcpName=Nubank%20MCP&mcpServerUrl=https%3A%2F%2Fapi.mcp.ai%2Fp_nubank)

Manual: [claude.ai/customize/connectors](https://claude.ai/customize/connectors) → **+** → **Add custom connector** → name `Nubank MCP`, URL `https://api.mcp.ai/p_nubank`.

### Cursor

[➕ Install in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=nubank&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9udWJhbmsifQ==)

### VS Code (Copilot Chat)

[➕ Install in VS Code](vscode:mcp/install?name=nubank&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_nubank%22%7D)
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_nubank` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
